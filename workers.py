    ...
    try:
        ...
        for user in users:
            ...
            if 'daily_playtime_reminder' in user_data:
                ...
                logger.info(f"Processing user {user.id} with reminder {reminder} in timezone {timezone_str}")
                user_id = user.id
                ...
                if push_ids:
                    ...
                    if last_sent_time and last_sent_time.date() == now.date():
                        logger.info(f"Notification for user {user_id} already sent today.")
                    elif time_to_send.total_seconds() <= 60:  # Send the notification if it's within the next minute
                        logger.info(f"Notification for user {user_id} should go out now: {format_timedelta(time_to_send)} left")
                        ...
                        logger.info(f"Notification for user {user_id} is scheduled in: {format_timedelta(time_to_send)}")
                else:
