...
class SearchParks:
    ...
    def _extract_park_details(self, place):
        ...
        return {
            ...
            'geohash': self.geohash(longitude=location_metadata['longitude'], latitude=location_metadata['latitude']),
            'address': place['formattedAddress'],
            'location': [location_metadata['latitude'], location_metadata['longitude']],
            'website_url': place['websiteUri'],
            'image': upload_image_to_azure(self.get_photo_url(place['photos'][0]['name']), place['displayName']['text']),
            'visitedBy': []
        }
        ...
        results = database.collection.find({
            "location": {
                "$near": {
                    "$geometry": {
                        "type": "Point",
                        "coordinates": [location_metadata['longitude'], location_metadata['latitude']]
                    },
                    "$maxDistance": radius
                }
            }
